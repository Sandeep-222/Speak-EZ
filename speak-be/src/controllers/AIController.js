...
const EnglishScore1 = async (req, res) => {
  const { userResponse } = req.body;

  if (!userResponse) {
    return res.status(400).json({ error: "No user response provided" });
  }

  try {
    const documents = [userResponse];
    const summary = [];

    // Define the summarization action
    const actions = [
      {
        kind: "ExtractiveSummarization",
        maxSentenceCount: 5,
    ...
    for await (const actionResult of results) {
      if (actionResult.kind !== "ExtractiveSummarization") {
        console.error(`Expected ExtractiveSummarization but got: ${actionResult.kind}`);
        continue;
      }
      if (actionResult.error) {
        console.error(`Action error: ${actionResult.error.code} - ${actionResult.error.message}`);
        continue;
      }

      for (const result of actionResult.results) {
        if (result.error) {
          console.error(`Document error: ${result.error.code} - ${result.error.message}`);
          continue;
        }
        result.sentences.forEach(sentence => summary.push(sentence.text));
      }
    }
    ...
    // Return response
    res.status(200).json({ misspelledWords, summary });
...
const RandomText=async(req,res)=>{
    const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
    const prompt = `Provide  descriptive text suitable for practicing English fluency and accent, focusing on the theme of adventure. so that a person takes 2 minutes to read it. Do not include any extra text except the text and dont use bold font. just give me the text without any further information `;
    try{
    async function run() {
